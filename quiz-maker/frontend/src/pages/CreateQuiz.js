...
const CreateQuiz = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.options.some(o => !o.text.trim())) return toast.error(`All option fields in question ${i + 1} must be filled`);
    }
    setSaving(true);
    try {
      const payload = {
    ...
    <div style={{ padding: '2.5rem 0 5rem' }} className="page-enter">
      <div className="container-md">
        <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: '2rem', marginBottom: '0.4rem' }}>Create a Quiz</h1>
          <p style={{ color: 'var(--text2)' }}>Build your quiz with multiple-choice questions</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.section}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="e.g. Ultimate Science Quiz" value={meta.title} onChange={setMF('title')} required />
            </div>
            <div className="form-group">
              <label>Description</label>
              <textarea className="form-input" rows={3} placeholder="Describe your quiz…" value={meta.description} onChange={setMF('description')} style={{ resize: 'vertical' }} />
            </div>
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="biology, cells, science" value={meta.tags} onChange={setMF('tags')} />
            </div>
          </div>
                  ...
                  {!isCollapsed && (
                    <div style={{ padding: '1.25rem', borderTop: '1px solid var(--border2)' }}>
                      <div className="form-group">
                        ...
                        <textarea className="form-input" rows={2} placeholder="Enter your question…" value={q.text} onChange={e => updateQ(qi, 'text', e.target.value)} style={{ resize: 'vertical' }} />
                      </div>
                        ...
                        {q.options.map((opt, oi) => (
                          <div key={oi} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                            <button
                              ...
                              style={{
                                ...styles.correctBtn,
                                background: opt.isCorrect ? 'var(--green)' : 'var(--bg3)',
                                borderColor: opt.isCorrect ? 'var(--green)' : 'var(--border2)',
                                color: opt.isCorrect ? '#fff' : 'var(--text3)',
                              }}
                            >
                              <FiCheck size={14} />
                            </button>
                      ...
                      <div className="form-group" style={{ marginBottom: 0 }}>
                        ...
                        <input className="form-input" placeholder="Explain why the correct answer is right…" value={q.explanation} onChange={e => updateQ(qi, 'explanation', e.target.value)} />
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
            ...
            <span style={{ color: 'var(--text3)', fontSize: '0.85rem', alignSelf: 'center' }}>{questions.length} question{questions.length !== 1 ? 's' : ''}</span>
          </div>
...
const styles = {
  section: {
    ...
    borderRadius: 'var(--radius)', padding: '1.75rem', marginBottom: '1.5rem',
  },
